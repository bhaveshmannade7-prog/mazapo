...
# FIX: Corrected import path for SearchClientSync in algoliasearch==3.0.0
...
if not BOT_TOKEN or not ALGOLIA_APP_ID or not ALGOLIA_SEARCH_KEY or not DATABASE_URL:
    logger.warning("⚠️  WARNING: Missing essential environment variables (DB/Token)")
    logger.warning("⚠️  Running in DEMO MODE - bot functionality will be limited")
    logger.warning("⚠️  For production, set: BOT_TOKEN, DATABASE_URL, ALGOLIA_APPLICATION_ID, ALGOLIA_SEARCH_KEY")
...
def initialize_db_and_algolia_with_retry(max_retries: int = 5, base_delay: float = 2.0) -> bool:
    ...
    for attempt in range(max_retries):
        try:
            ...
            algolia_client = SearchClientSync(ALGOLIA_APP_ID, ALGOLIA_SEARCH_KEY)
...
async def handle_search(message: Message):
    try:
        ...
        results = await algolia_fuzzy_search(query, limit=20) # AWAIT is essential for async wrapper
    ...
    try:
        ...
        if message.document or message.video:
            ...
            if title and title != "Unknown Movie" and post_id:
                await add_movie_to_db_and_algolia(title, post_id) # AWAIT is essential for async wrapper
    except Exception as e:
...
async def cmd_cleanup_users(message: Message):
    ...
    await message.answer(f"🧹 Cleaned up in-memory user list. Cleared {old_count} entries.")
...
async def cmd_total_movies(message: Message):
    ...
    try:
        ...
        if db_session:
            ...
            db_session.close()
            await message.answer(f"📊 Live Indexed Movies in DB: {count}")
        else:
...
async def start_bot():
    ...
    if DEMO_MODE or not bot:
        ...
        logger.warning("⚠️  Set BOT_TOKEN, DATABASE_URL, ALGOLIA_APPLICATION_ID, ALGOLIA_SEARCH_KEY to activate bot")
        while True:
