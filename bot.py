...
def initialize_db_and_algolia():
    ...
    try:
        ...
        core_key_str = FIREBASE_PRIVATE_KEY_JSON_STR # Use the environment variable content
        ...
        # 1. Clean up and load the private key string into a dictionary
        clean_key_str = core_key_str.strip().replace('\\n', '\n').strip()
        
        # We try to clean invisible characters and find the start of JSON manually if necessary
        try:
            cred_dict = json.loads(clean_key_str)
        except json.JSONDecodeError as e:
            start_index = clean_key_str.find('{')
            if start_index != -1:
                cleaned_json_string = clean_key_str[start_index:]
                cred_dict = json.loads(cleaned_json_string)
            else:
                raise json.JSONDecodeError(f"Could not find JSON start in key string.", core_key_str, 0)
        ...
        print("HINT: Please ensure the FULL JSON CREDENTIALS OBJECT is pasted correctly.")
        return False
...
# ... (All core logic and admin commands remain the same)
