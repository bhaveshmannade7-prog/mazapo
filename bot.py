...
def initialize_db_and_algolia():
    ...
    try:
        ...
        # --- ROBUST FIREBASE KEY HANDLING FIX ---
        # 1. We must load the entire JSON object, even if Render adds quotes/spaces.
        # This function attempts to parse the string to a dict using a more forgiving approach.
        
        # We assume the user has pasted the full JSON object as a string in the environment variable.
        # The key string provided by the user needs to be manually converted into a full credentials dictionary.
        
        # Manually reconstruct the credentials dictionary based on the known format and provided data
        # This bypasses the direct JSON load error on the entire single-line string.
        
        # The core private key string provided by the user is:
        core_key_string = FIREBASE_PRIVATE_KEY_JSON_STR.strip().replace('\\n', '\n')
        
        # Manually constructing the credentials dictionary from the known variables
        # We assume the user has pasted the correct full JSON object as a single line string. 
        # We MUST ensure that the user pastes the FULL JSON OBJECT (starting with { and ending with }) 
        # in the environment variable, not just the private key value.
        
        # Since the error suggests line 1 column 1 error, the user is likely pasting the raw text, 
        # which is not valid JSON. We will revert to the safest method which requires the user 
        # to paste the full credentials object:
        
        try:
            # First, try to load the full JSON object assuming it starts with {
            cred_dict = json.loads(core_key_str)
        except json.JSONDecodeError:
            # If that fails, it means Render is adding extra characters. We will try to clean it.
            # This is the last resort against strange invisible characters.
            cleaned_json_string = core_key_str.strip()
            
            # This assumes the user has pasted the full JSON object with correct newlines.
            # Since the user pasted the full JSON object including the "type" field before:
            if not cleaned_json_string.startswith('{'):
                # This means the error is due to an invisible character before {. 
                # We try to find the start of the JSON manually.
                start_index = cleaned_json_string.find('{')
                if start_index != -1:
                    cleaned_json_string = cleaned_json_string[start_index:]
        ...
        print("HINT: Please re-paste the FULL JSON CREDENTIALS OBJECT as a single line in Render.")
        return False
...
# ... (All other core logic functions remain the same: check_rate_limit, add_user, algolia_fuzzy_search, add_movie_to_db_and_algolia)
...
async def handle_search(message: Message):
    try:
        ...
        if user_id not in ADMIN_IDS and user_id not in verified_users: await cmd_start(message); return
...
async def send_movie_link(callback: types.CallbackQuery):
    try:
        ...
        if user_id not in ADMIN_IDS and user_id not in verified_users: await callback.answer("🛑 पहुँच वर्जित (Access Denied)。"); return

        try: post_id = int(callback.data.split('_')[1])
        except (ValueError, IndexError): await callback.answer("❌ गलत चुनाव।"); return
