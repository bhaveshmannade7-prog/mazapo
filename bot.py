...
FIREBASE_PRIVATE_KEY = os.getenv("FIREBASE_PRIVATE_KEY") # This holds the full JSON object string (Safely from ENV)
...
def initialize_db_and_algolia():
    ...
    try:
        ...
        # --- ROBUST FIREBASE KEY HANDLING (The Final Secure Fix) ---
        key_value = os.getenv("FIREBASE_PRIVATE_KEY")
        
        # 1. Aggressively clean the string of Render's common artifacts
        clean_key_str = key_value.strip().strip('"') # Remove external quotes/spaces
        
        # 2. Process the string to make it loadable JSON
        processed_json_string = clean_key_str.replace('\\n', '\n').replace('\\"', '"')

        # 3. Find the starting '{' and load the JSON object
        start_index = processed_json_string.find('{')
        if start_index == -1:
             raise ValueError("Could not find the starting '{' bracket in the Firebase key string.")

        cred_dict = json.loads(processed_json_string[start_index:])
...
# New Feature 1: Cleanup Inactive Users
...
# New Feature 2: Reload Config (Dummy for Render)
