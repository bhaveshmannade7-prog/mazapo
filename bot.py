...
FIREBASE_CLIENT_EMAIL = os.getenv("FIREBASE_CLIENT_EMAIL") # New Key from .env
FIREBASE_PRIVATE_KEY_STR = os.getenv("FIREBASE_PRIVATE_KEY") # New Key from .env
...
# FATAL CHECK IS UPDATED to reflect the new variable names
...
def initialize_db_and_algolia():
    ...
    try:
        ...
        # --- ROBUST FIREBASE KEY HANDLING FIX for Individual Keys ---
        
        # 1. Create the credential dictionary from separate keys
        # We strip quotes that Render might add to the Private Key string
        private_key_clean = FIREBASE_PRIVATE_KEY_STR.strip().strip('"')
        ...
        cred_dict = {
            ...
            # Other fields are not strictly necessary for python SDK init with credentials.Certificate
        }
        ...
        print("HINT: Please check if FIREBASE_PRIVATE_KEY is in correct format (with internal \n) and other essential keys are present.")
        return False
...
# --- TELEGRAM HANDLERS (All features complete) ---
