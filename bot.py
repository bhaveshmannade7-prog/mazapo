...
def initialize_db_and_algolia():
    ...
    try:
        ...
        # --- ROBUST FIREBASE KEY HANDLING (Final Fix) ---
        key_value = os.getenv("FIREBASE_PRIVATE_KEY")
        
        # 1. Clean the string: remove surrounding quotes/spaces
        clean_key_str = key_value.strip().strip('"')
...
# --- ADMIN COMMANDS (New Features Integrated) ---
...
async def cmd_cleanup_users(message: Message):
    ...
    # NOTE: This only clears the in-memory dictionary 'users_database'. 
    # For a persistent fix, this should clear Firestore/DB.
...
async def cmd_reload_config(message: Message):
    ...
    # In Render, environment variables are only read at startup. 
    # The actual reload requires a restart, but we provide status update.
