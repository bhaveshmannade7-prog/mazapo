...
import sys 
import traceback
import logging # FIX: Added for Flask logging control
...
if __name__ == "__main__":
    ...
    # Start Flask in a separate daemon thread (Runs in background)
    flask_thread = threading.Thread(target=start_flask_server, daemon=True)
    flask_thread.start()
    print("✅ Flask health check server started in background")
    
    # FIX 2: Increased delay to 3 seconds to ensure Flask thread starts properly before main async loop
    time.sleep(3) 
    ...
    try:
        ...
        # If bot fails, main process still enters infinite loop for Flask's sake (though Flask is already threaded)
        try:
            while True:
                time.sleep(1)
        except KeyboardInterrupt:
