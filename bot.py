...
# ALL ESSENTIAL LIBRARIES ARE USED HERE
...
ADMIN_IDS = [7263519581] # Admin ID MUST be an integer
...
# Critical check for essential variables
...
def initialize_db_and_algolia():
    ...
    try:
        ...
        # --- ROBUST FIREBASE KEY HANDLING (The Fix for JSONDecodeError) ---
        key_value = os.getenv("FIREBASE_PRIVATE_KEY")
        
        # 1. Clean the string: aggressively remove external quotes/spaces
        clean_key_str = key_value.strip()
        if clean_key_str.startswith('"') and clean_key_str.endswith('"'):
            clean_key_str = clean_key_str[1:-1].strip()
        
        # 2. We assume the user has pasted the correct FULL JSON OBJECT string
        # We manually try to handle Render's common mistake of adding extra characters.
        ...
        # 3. Load the cleaned JSON dictionary
        ...
        print("HINT: Ensure the FULL JSON OBJECT is pasted correctly without extra leading/trailing characters.")
        return False
