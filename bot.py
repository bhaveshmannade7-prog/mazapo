...
async def add_movie_to_db_and_algolia(title: str, post_id: int):
    ...
    def sync_data(db_session):
        try:
            ...
            # 2. Add to PostgreSQL (Master data)
            new_movie = Movie(title=title.strip(), post_id=post_id)
            db_session.add(new_movie)
            db_session.commit()
            db_session.refresh(new_movie) # Get the primary key (id)

            # 3. Add to Algolia Index (Search data)
            algolia_index.save_object({
                "objectID": new_movie.id, # Use SQL Primary Key as Algolia's object ID
                "title": title.strip(),
                "post_id": post_id,
            })
    ...
    await asyncio.to_thread(sync_data, db_session)
...
async def cmd_total_movies(message: Message):
    if not message.from_user or message.from_user.id not in ADMIN_IDS: return
    if not engine: await message.answer("❌ Database connection failed."); return
    try:
        db_session = next(get_db())
