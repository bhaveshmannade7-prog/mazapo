...
def initialize_db_and_algolia():
    ...
    try:
        ...
        # --- ROBUST FIREBASE KEY HANDLING FIX (Designed to bypass Render's JSON parsing issues) ---
        key_value = os.getenv("FIREBASE_PRIVATE_KEY")
        
        # 1. Aggressively strip all surrounding whitespace and external quotes
        clean_key_str = key_value.strip()
        if clean_key_str.startswith('"') and clean_key_str.endswith('"'):
            clean_key_str = clean_key_str[1:-1].strip()

        # 2. Find the start of the JSON object ({) and ignore any preceding garbage characters
        start_index = clean_key_str.find('{')
        if start_index == -1:
             raise ValueError("Could not find the starting '{' bracket in the Firebase key string.")

        # 3. Process the cleanup key to handle escaped newlines/quotes correctly
        processed_json_string = clean_key_str[start_index:]
        
        # 4. Load the cleaned JSON dictionary
        ...
        print("HINT: The JSON structure inside the key is broken or contains an invalid character.")
        return False
