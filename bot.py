...
# NEW: Add keys necessary for the Service Account Certificate (to fix missing token_uri)
...
def initialize_db_and_algolia():
    ...
    try:
        ...
        # 1. Clean the Private Key and reconstruct the FULL Service Account DICTIONARY
        private_key_clean = os.getenv("FIREBASE_PRIVATE_KEY").strip().strip('"').replace('\\n', '\n')
        
        # We manually construct the dictionary with ALL required fields
        cred_dict = {
            "type": "service_account",
            "project_id": FIREBASE_PROJECT_ID,
            "private_key": private_key_clean,
            "client_email": FIREBASE_CLIENT_EMAIL,
            "token_uri": FIREBASE_TOKEN_URI, # FIX: Added missing token_uri
            "auth_uri": FIREBASE_AUTH_URI,   # Added for completeness
        }
...
# ... (rest of the code for logic and handlers remains the same)
